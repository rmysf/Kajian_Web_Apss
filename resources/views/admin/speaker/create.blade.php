<x-admin-layout>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8">
        <form action="{{ route('admin.speaker.store') }}" method="POST" enctype="multipart/form-data" x-data="photoPreview()">
            @csrf
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full rounded-md border-gray-200 shadow-sm focus:border-brand-emerald-900 focus:ring focus:ring-brand-emerald-900 focus:ring-opacity-50" required placeholder="Contoh: Ustadz Dr. Syafiq Riza Basalamah, M.A.">
                            @error('name')
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="6" class="w-full rounded-md border-gray-200 shadow-sm focus:border-brand-emerald-900 focus:ring focus:ring-brand-emerald-900 focus:ring-opacity-50 resize-none overflow-y-auto" placeholder="Tuliskan biografi singkat atau latar belakang pemateri...">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
